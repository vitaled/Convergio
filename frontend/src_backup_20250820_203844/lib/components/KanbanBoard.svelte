<script lang="ts">
	...
	let columns: Column[] = [
		{ id: 'pending', title: 'To Do', color: 'bg-gray-100', tasks: [] },
		{ id: 'in_progress', title: 'In Progress', color: 'bg-blue-100', tasks: [] },
		{ id: 'in_review', title: 'In Review', color: 'bg-yellow-100', tasks: [] },
		{ id: 'completed', title: 'Done', color: 'bg-green-100', tasks: [] }
	];
		...
		{#each columns as column (column.id)}
			<div class="kanban-column {column.color} rounded-lg p-4">
				<div class="column-header mb-4">
					...
					<span class="text-sm text-gray-600">{column.tasks.length} items</span>
				</div>
					...
					{#each column.tasks as task (task.id)}
						...
						>
							...
							<div class="task-meta space-y-2">
								...
								{#if task.progress > 0}
									<div class="progress-bar bg-gray-200 rounded-full h-1.5">
										<div
