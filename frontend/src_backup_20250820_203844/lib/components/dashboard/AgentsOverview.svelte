...
<div class="bg-white border border-gray-200 rounded">
  <div class="px-4 py-3 border-b border-gray-200">
    <div class="flex items-center justify-between">
      <h3 class="text-sm font-medium text-gray-900">AI Agents Ecosystem</h3>
      <button 
    ...
    {:else}
      ...
      <div class="grid grid-cols-4 gap-4 mb-6">
        ...
        <div class="bg-gray-50 p-3 rounded">
          <p class="text-xl font-bold text-gray-600">{agents.filter(a => a.status === 'inactive').length}</p>
      ...
      {#if agents.length > 0}
        <div class="space-y-3">
          <h4 class="text-xs font-medium text-gray-700">Active Agents</h4>
          {#each agents.slice(0, 8) as agent}
            <div class="flex items-center justify-between p-3 border border-gray-100 rounded hover:bg-gray-50">
              <div class="flex items-center space-x-3">
                ...
                <div>
                  <p class="text-sm font-medium text-gray-900">{agent.name || agent.agent_key}</p>
          ...
          {#if agents.length > 8}
            <div class="text-center">
              <p class="text-xs text-gray-500">... and {agents.length - 8} more</p>
            </div>
          {/if}
        </div>
      {:else}
