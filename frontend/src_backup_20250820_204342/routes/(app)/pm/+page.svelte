...
<div class="min-h-screen bg-surface-900 dark:bg-surface-100 space-y-6">
	...
	<div class="flex items-center justify-between">
		<div>
			<h1 class="text-lg font-medium text-surface-100 dark:text-surface-900">Projects</h1>
			<p class="mt-1 text-sm text-surface-300 dark:text-surface-700">Manage AI agent projects and coordinate workflows</p>
		</div>
	...
	<div>
		...
		{:else}
			...
			{#if projects.length > 0}
				<div class="bg-surface-950 dark:bg-surface-50 border border-surface-700 dark:border-surface-300 rounded-lg overflow-hidden">
					<div class="p-4 border-b border-surface-700 dark:border-surface-300 bg-surface-900 dark:bg-surface-100">
						<h2 class="text-md font-medium text-surface-100 dark:text-surface-900">Select Project</h2>
					</div>
							...
							{#each projects as project}
								<button
									...
									class="text-left p-4 rounded-lg border-2 transition-all duration-200 hover:shadow-md {selectedProjectId === project.id 
										...
										: 'border-surface-700 dark:border-surface-300 bg-surface-950 dark:bg-surface-50 hover:border-surface-600 dark:border-surface-400'}"
								>
									<div class="font-medium text-surface-100 dark:text-surface-900 mb-1">{project.title}</div>
									<div class="text-sm text-surface-400 dark:text-surface-600">{project.description || 'No description'}</div>
									<div class="mt-2">
			...
			{:else}
				<div class="bg-surface-950 dark:bg-surface-50 border border-surface-700 dark:border-surface-300 rounded-lg">
					<div class="text-center py-20">
						<div class="w-24 h-24 mx-auto mb-6 bg-surface-800 dark:bg-surface-200 rounded-full flex items-center justify-center">
							<svg class="w-12 h-12 text-gray-400" fill="none" stroke="currentColor" viewBox="0 0 24 24">
								<path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M19 11H5m14 0a2 2 0 012 2v6a2 2 0 01-2 2H5a2 2 0 01-2-2v-6a2 2 0 012-2m14 0V9a2 2 0 00-2-2M5 11V9a2 2 0 012-2m0 0V5a2 2 0 012-2h6a2 2 0 012 2v2M7 7h10" />
							</svg>
						</div>
						<h3 class="text-xl font-medium text-surface-100 dark:text-surface-900 mb-2">No Projects Yet</h3>
						<p class="text-surface-400 dark:text-surface-600 mb-6">Create your first project to get started with project management</p>
						<button 
			...
			{#if selectedProjectId && projects.length > 0}
				<div class="bg-surface-950 dark:bg-surface-50 border border-surface-700 dark:border-surface-300 rounded-lg overflow-hidden">
					<div class="p-4 border-b border-surface-700 dark:border-surface-300 bg-surface-900 dark:bg-surface-100">
						<div class="flex items-center justify-between">
							<h2 class="text-md font-medium text-surface-100 dark:text-surface-900">Project Views</h2>
							
							<!-- View Tabs -->
							<div class="flex bg-surface-800 dark:bg-surface-200 rounded-lg p-1">
								<button
									...
									class="px-4 py-2 rounded-md text-sm font-medium transition-all duration-200 {selectedView === 'kanban' 
										? 'bg-surface-950 dark:bg-surface-50 text-surface-100 dark:text-surface-900 shadow-sm' 
										: 'text-surface-400 dark:text-surface-600 hover:text-surface-100 dark:text-surface-900'}"
								>
									Kanban Board
								</button>
								<button
